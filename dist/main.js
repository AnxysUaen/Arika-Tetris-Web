import { blockTypes, blockColors } from "./tetris/block";
...
let timer = 0; // 定时器
let currentBlocks = []; // 当前方块组
let currentBlock = null; // 当前活动方块
let canvasData = null; // 画布数据
function gameLoop() {
    // 生成一组方块，顺序随机
    if (currentBlocks.length === 0) {
        ...
        randomBlocks.forEach((type, index) => {
            ...
            if (block.type === 'O')
                block.position = { x: 4, y: 0 }; // O方块位置调整
